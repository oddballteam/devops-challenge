...
resource "aws_iam_role_policy_attachment" "ecs_task" {
  count      = length(var.task_role_arn) == 0 ? length(var.task_policy_arns) : 0
...
data "aws_iam_policy_document" "ecs_task_exec" {
  count = length(var.task_exec_role_arn) == 0 ? 1 : 0
...
resource "aws_iam_role_policy_attachment" "ecs_exec" {
  count      = length(var.task_exec_role_arn) == 0 ? length(var.task_exec_policy_arns) : 0
...
resource "aws_security_group_rule" "allow_icmp_ingress" {
  count             = var.enable_icmp_rule ? 1 : 0
